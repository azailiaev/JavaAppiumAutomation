...
public class MainPageObject {
    ...
    public boolean waitForElementNotPresent(String locator, String error_message, long timoutInSeconds)
    {
        By by = this.getLocatorByString(locator);
    ...
    public boolean isElementLocatedOnTheScreen(String locator){
        int elemen_location_by_y = this.waitForElementPresent(locator, "Cannot find element by locator", 10).getLocation().getY();
        if (Platform.getInstance().isMW()){
            JavascriptExecutor JSExecutor = (JavascriptExecutor) driver;
            Object js_result = JSExecutor.executeScript("return window.pageYOffset");
            elemen_location_by_y -= Integer.parseInt(js_result.toString());
        }
        int screen_size_by_y = driver.manage().window().getSize().getHeight();
        return elemen_location_by_y < screen_size_by_y;
    }
    ...
    public void clickElementToTheRightUpperCorner(String locator, String error_message){
        if (driver instanceof AppiumDriver){
            ...
            int point_to_click_x = (right_x + width) +230;
